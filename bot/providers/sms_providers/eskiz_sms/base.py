...
from bot.providers.sms_providers.eskiz_sms.request import Request
...
class EskizSMSBase(metaclass=Meta):
    ...
    @classmethod
    def count_sms_chars(cls, message: str):
        length = len(message)
        print(f"Sms chars count: {length}")
        if 0 <= length <= 160:
            spend = 1
        elif 160 < length <= 306:
            spend = 2
            ...
            spend = 6
        elif 918 < length <= 1071:
            spend = 7
        elif 1071 < length <= 1224:
            spend = 8
        else:
            spend = 30
        return spend
